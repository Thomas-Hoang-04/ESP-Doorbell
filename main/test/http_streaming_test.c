...
static esp_err_t capture_handler(httpd_req_t *req) {
    ...
    ret = httpd_resp_send(req, (const char *)fb->buf, (ssize_t)fb->len);
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while (true) {
        ...
        const ssize_t hlen = snprintf(part_buf, sizeof(part_buf),
        ...
        ret = httpd_resp_send_chunk(req, (const char *)fb->buf, (ssize_t)fb->len);
        if (ret != ESP_OK) {
            ESP_LOGE(CAMERA_TAG, "Failed to send chunk");
            esp_camera_fb_return(fb);
            return ret;
        ...
        vTaskDelay(pdMS_TO_TICKS(42)); // 33ms = 30fps
    }
